...
describe('database layer', () => {
  ...
  test('organic_standards table exists', () => {
    const result = db.all<{ name: string }>(
  ...
  test('cover_crops table exists', () => {
    const result = db.all<{ name: string }>(
      "SELECT name FROM sqlite_master WHERE type='table' AND name='cover_crops'"
    );
    expect(result).toHaveLength(1);
  });

  test('soil_health table exists', () => {
    const result = db.all<{ name: string }>(
